...
class NeuralNetwork:
    ...
    def _backprop(self, y):
        ...
        for i in range(len(self.activations) - 2, 0,-1):
            delta_i = self.theta[i][:, 1:].T @ delta_i * (self.activations[i][1:] * (1 - self.activations[i][1:]))
            delta.append(delta_i)
        ...
        for i in range(len(delta) - 1, -1, -1):
            Delta[i][:, 1:] =  np.transpose(self.activations[i][1:,:] @ delta[i].T)
        
        
        # Create matrix D from Δ matrix, there is some regularization here
        D = (1.0 / m) * np.array(Delta, dtype="object")
        

        for i in range(0, len(D)):
            d = D[i]
            t = self.theta[i]
            d[:,1:] += self.regularization_rate * t[:, 1:]
        ...
        for i in range(0, len(self.theta)):
            t = self.theta[i]
            d = D[i]
            t[:,1:] = t[:,1:] - self.learning_rate * d[:, 1:]
            self.theta[i] = t
    ...
    def _cost_function(self, y):
        ...
        m = y.shape[1]
        hyp = self.activations[-1]

     
        cost = -1.0 / m * np.sum(y * np.log(hyp, out=np.zeros_like(hyp), where=(hyp != 0)) 
        + (1 - y) * np.log(1 - hyp, out=np.zeros_like(hyp), where=((1- hyp) != 0))) + self._cost_regularize(m)
    ...
    def _cost_regularize(self, m):
        partial = 0
        for i in range(0, len(self.theta)):
            aux = np.square(self.theta[i][:, 1:])
            partial += np.sum(aux)
        
        return self.regularization_rate / (2.0 * m) * partial
